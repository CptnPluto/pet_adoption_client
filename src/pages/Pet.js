...
const Pet = () => {
	...
	const { setSavedPetIds, savedPetIds, isSaved, setIsSaved } = usePetsListContext();
			...
			{!pet || !message ? (
				...
				<>
					...
					<div className="pet-container">
						<img src={pet.picture} width="150" height="150" alt="pet" />
						<div className="details">
							...
							<div className="label">
								Hypoallergenic: {pet.hypoallergenic ? pet.hypoallergenic : "No"}
							</div>
							<div className="label">Dietary Restrictions: {pet.dietaryRes}</div>
							...
							{user && (
								<>
									...
									{pet.ownerId === user.id ? (
										...
										<>
											{!savedPetIds.includes(pet.petId) ? (
												<button
													type="button"
													value="Saved"
													onClick={savePet}
						...
						{show && (
							<Modal show={show} onClose={() => setShow(false)}>
								<PetForm handleSubmit={handleSubmit} petData={pet} />
							</Modal>
						)}
					</div>
				</>
